...
If you navigate to the PT Exams benchmark, you'll notice that [Qwen3.5 2B](https://huggingface.co/Qwen/Qwen3.5-2B) actually beats AMALIA. This is suprising, that model has a fourth(!) of AMALIA's parameters/size.
...
What's the takeaway? In practice, if you're a law office and interested in a small model that can help you respond to legal questions, you might be better off starting with [Qwen3.5 4B](https://huggingface.co/Qwen/Qwen3.5-4B) than AMALIA (select [LegalBench PT](https://huggingface.co/datasets/BeatrizCanaverde/LegalBench.PT) above). It's thanks to the AMALIA team and their great work that we now have something to judge these models against. 

Still, I think this tells something else that is interesting for the future of AMALIA. Could we have a much smaller model that everyone could run on their own hardware easily, but that still outperforms smaller models? Should the next AMALIA be a reasoning model? 

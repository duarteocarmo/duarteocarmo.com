...
async function callApi({ context, resource, query = "" }) {
...
async function callTool({ context, name, args }) {
  ...
  if (name === "get_profile") return callApi({ context, resource: "profile" });
  if (name === "list_pages") return callApi({ context, resource: "pages" });
...
export async function onRequest(context) {
  ...
  if (request.jsonrpc !== "2.0" || typeof request.method !== "string") {
    return rpcError({ id: request.id, code: -32600, message: "Invalid JSON-RPC request." });
  }
  ...
  if (request.method === "initialize") {
    ...
    return rpcResponse({
      ...
      result: {
        ...
        instructions: "Use these read-only tools to find public content on duarteocarmo.com.",
      },
    });
  }
  if (request.method === "ping") return rpcResponse({ id: request.id, result: {} });
  ...
  return rpcError({ id: request.id, code: -32601, message: "Method not found." });
}
